...
const OrderConfirmations = () => {
  ...
  const orderConfirmations = useMemo(() => {
    return invoices.filter(inv => (inv as any).document_type === 'order_confirmation');
  }, [invoices]);

  const filtered = useMemo(() => {
    if (statusFilter === 'all') return orderConfirmations;
    return orderConfirmations.filter(inv => inv.status === statusFilter);
  }, [orderConfirmations, statusFilter]);
      ...
      <FeatureGate feature="invoiceModule">
        <div className="p-6 lg:p-8 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Button asChild>
              <Link to="/invoices/new?type=order_confirmation">
              ...
              {loading ? (
                ...
                <div className="text-center py-12 space-y-3">
                  ...
                  <Button asChild variant="outline">
                    <Link to="/invoices/new?type=order_confirmation">Erste Auftragsbestätigung erstellen</Link>
                  </Button>
                </div>
              ) : (
                <Table>
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" onClick={e => e.stopPropagation()}>
                                ...
                                {inv.status === 'sent' && (
                                  <DropdownMenuItem onClick={() => updateInvoiceStatus(inv.id, 'confirmed')}>
                                    <CheckCircle className="h-4 w-4 mr-2" /> Als bestätigt markieren
                                  </DropdownMenuItem>
                                )}
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Die Auftragsbestätigung {inv.invoice_number} wird unwiderruflich gelöscht.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
